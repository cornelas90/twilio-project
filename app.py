...
countries = ['KOR','PAN','MEX','ENG','COL','JPN','POL','SEN','RUS','EGY','POR','MAR','URU','KSA','IRN','ESP','DEN',
			'AUS','FRA','PER','ARG','CRO','BRA','CRC','NGA','ISL;','SRB','SUI','BEL','TUN','GER','SWE']
	...
	if body == 'today':
		...
		for match in html:
			if match['status'] == 'completed':
				output += (match['home_team']['country'] + ' ' + str(match['home_team']['goals']) +
				 ' vs ' + match['away_team']['country'] + ' ' + str(match['away_team']['goals']) + '\n')
		output += '\n\n--- Future Matches ---\n'
		for match in html:
		...
		You can response with "today" to get today\'s details, 
		"tomorrow" to get tomorrow\'s details, "complete" to get the group stage standing of teams, 
		or you can reply with a country FIFA code (BRA, ARG, etc.) and we will send you the standing of that particular country. 
		or a list of FIFA codes send "list". \n\nHave a great day! ''')
